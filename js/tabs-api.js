// Tabs functionality for matches section with API integration
class TabsComponent {
    constructor() {
    ...
    async loadTabContent(tabId) {
        ...
        // Show loading state
        panel.innerHTML = this.createLoadingState();
        ...
        try {
            ...
            switch (tabId) {
                case 'live':
                    matches = await this.apiService.getLiveMatches();
                    break;
                case 'today':
                    matches = await this.apiService.getTodayMatches();
                    break;
                case 'upcoming':
                    matches = await this.apiService.getUpcomingMatches();
                    break;
                default:
            ...
            console.error(`Error loading ${tabId} matches:`, error);
            
            // Fallback to mock data
            const matches = this.filterMockMatches(mockData.matches, tabId);
    ...
    filterMockMatches(matches, status) {
        switch (status) {
            case 'live':
    ...
    createLoadingState() {
        return `
            <div class="loading-state" style="text-align: center; padding: 3rem 1rem; color: var(--gray-500);">
                <div style="width: 40px; height: 40px; border: 3px solid var(--gray-200); border-top: 3px solid var(--primary-blue); border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 1rem;"></div>
                <p style="font-size: var(--font-size-sm);">Carregando partidas...</p>
            </div>
        `;
    }

    createMatchCard(match) {
        const homeInitials = this.getTeamInitials(match.homeTeam);
        const awayInitials = this.getTeamInitials(match.awayTeam);
    ...
    addMatchClickHandlers(panel) {
        ...
        matchCards.forEach(card => {
            card.addEventListener('click', async () => {
                ...
                await this.handleMatchClick(matchId);
            });
        });
    }

    async handleMatchClick(matchId) {
        try {
            // Try to get match details from API
            const match = await this.apiService.getMatch(matchId);
    ...
    async updateMatch(matchId, newData) {
        // In API version, this would make an API call to update the match
        // For now, we'll just refresh the active tab
